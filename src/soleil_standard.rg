...
local DOM = (require 'dom_standard-desugared')(MAX_ANGLES_PER_QUAD, Radiation_columns, SCHEMA)
...
local function mkInstance() local INSTANCE = {}
  ...
  function INSTANCE.DeclSymbolsAgain(config) return rquote
...
--    where reads writes(points) do
  C.printf("In stop timer\n");
  return --points
end

task start_timer() --points : region(ispace(int2d), point))
--    where reads writes(points) do
  C.printf("In start timer\n");
  return --points
end

task printElapsed(config: Config, t : float)
...
regentlib.saveobj(main, "soleil_standard.o", "object", MAPPER.register_mappers)
