...
#include "soleil_mapper_specified.h"
...
static Realm::Logger LOG("soleil_mapper_specified");
...
public:
  SoleilMapper(Runtime* rt, Machine machine, Processor local)
    : SpecifiedMapper(rt->get_mapper_runtime(), machine, local, "soleil_mapper_specified"),
      all_procs_(remote_cpus.size()) {

    name_application("soleil_specified");
    ...
    if (task.is_index_space ||
      ...
      bool success = runtime->retrieve_semantic_information
        (ctx, region, SAMPLE_ID_TAG, info, info_size,
         false, true); // can_fail, wait_until_ready
  ...
  virtual void map_copy(const MapperContext ctx,
    ...
    dst_constraints.add_constraint
      (FieldConstraint(dst_req.privilege_fields,
                       false, false)); //contiguous, inorder
    CHECK(runtime->find_physical_instance
            (ctx, target_memory, dst_constraints,
             ...
             true, false), // acquire, tight_region_bounds
...
static void create_mappers(Machine machine,
  ...
  rt->replace_default_mapper(new SoleilMapper(rt, machine, *local_procs.begin()), Processor::NO_PROC);
/*
